(() => {
  ...
  const BRICK_HEIGHT = parseInt(
    computedStyle.getPropertyValue("--brick-height")
  );
  const BRICK_MARGIN = parseInt(
    computedStyle.getPropertyValue("--brick-margin")
  );
  const CONTAINER_WIDTH =
    BRICK_WIDTH * NUM_BRICK_COLUMN + BRICK_MARGIN * (NUM_BRICK_COLUMN + 1);
  ...
  class Game {
    ...
    initialize() {
      ...
      this.balls.push(
        new Ball(this.ball_speed, 1, 0, this.startCoord.x, this.startCoord.y)
      );
      this.startBall = this.balls[0];
      this.guideball = new Ball(
        this.ball_speed,
        1,
        0,
        this.startCoord.x,
    ...
    mouseMoveHandler(e) {
      ...
      let brickCollide =
        this.guideball.checkBrickCollision(nearBricks).length > 0;
      while (true) {
        ...
        brickCollide =
          this.guideball.checkBrickCollision(
            this.getNearbyBricksFromBall(this.guideball)
          ).length > 0;
      }
    ...
    mouseClickHandler(e) {
      ...
      if (e.button === 2 || !BALL_DEBUG) {
        ...
        this.ballAdditionId = setInterval(
          this.addOneBall.bind(this),
          (this.ballAddDelay * 1000) / FPS
        );
        this.animationId = setInterval(
          this.onFrameUpdate.bind(this),
          1000 / FPS
        );
      } else {
        // BALL DEBUGGING
    ...
    getNearbyBricksFromBall(ball) {
      ...
      if (Math.sin(ball.theta) <= 0) {
        ...
        if (Math.cos(ball.theta) <= 0) {
          ...
          // 1사분면
          nearDirecton = nearDirecton.map((elem) => [
            elem[0] * -1,
            elem[1] * -1,
          ]);
        }
      ...
      nearDirecton.forEach((direction) => {
        ...
        if (
          this.bricks[near_idy][near_idx] &&
          this.bricks[near_idy][near_idx].type === "brick"
        ) {
    ...
    onFrameUpdate() {
      ...
      this.balls.forEach((ball, index) => {
        if (ball.alive) {
          ...
          const bx = ball.x;
          const by = ball.y;
          const colliedBricks = ball.moveOneStep(
            this.getNearbyBricksFromBall(ball)
          );
          colliedBricks.forEach((brick) => {
            ...
            if (brick.durability <= 0) {
              if (brick.durability < 0) {
                ...
                const splited_ball = new Ball(
                  ball.speed,
                  Math.abs(brick.durability),
                  ball_theta,
                  bx,
                  by
                );
  ...
  class Ball {
    ...
    checkBrickCollision(nearbyBricksArr) {
      ...
      const reflectCorner = (
        ...
        let normalAngle = Math.atan2(
          this.y * y_flip - cornerY,
          this.x * x_flip - cornerX
        );
        if (x_flip < 0) {
      ...
      */
      ...
      switch (caseBit) {
        ...
        case 0b001:
          ...
          if (b_left.isColliedRightside(bx, by, x_flip, y_flip)) {
            ...
            b_left.isColliedCorner(CORNER_RIGHT_UP, bx, by, x_flip, y_flip)
          ) {
            colliedBricks.push(b_left);
        ...
        case 0b010:
          ...
          if (b_up.isColliedDownside(bx, by, x_flip, y_flip)) {
            ...
            b_up.isColliedCorner(CORNER_LEFT_DOWN, bx, by, x_flip, y_flip)
          ) {
            colliedBricks.push(b_up);
        ...
        case 0b111:
          ...
          const b_left_collied = b_left.isColliedRightside(
            bx,
            by,
            x_flip,
            y_flip
        ...
        case 0b100:
          ...
          if (b_upleft.isColliedDownside(bx, by, x_flip, y_flip)) {
            ...
            b_upleft.isColliedCorner(CORNER_RIGHT_DOWN, bx, by, x_flip, y_flip)
          ) {
            colliedBricks.push(b_upleft);
      ...
      if (b_downleft && !b_left) {
        /* 	 ?  ?
          ...
          ■    */
        if (
          b_downleft.isColliedCorner(CORNER_RIGHT_UP, bx, by, x_flip, y_flip)
        ) {
      ...
      if (b_upright && !b_up) {
        /* 	?  □ ■ 
          ?  ●     */
        if (
          b_upright.isColliedCorner(CORNER_LEFT_DOWN, bx, by, x_flip, y_flip)
        ) {
    ...
    checkItemCollision(item) {
      const distance2 =
        Math.pow(item.x - this.x, 2) + Math.pow(item.y - this.y, 2);
  ...
  class Brick {
    ...
    isColliedDownside(bx, by, x_flip, y_flip) {
      ...
      let bound_y = [
        refer_y + BRICK_HEIGHT,
        refer_y + BRICK_HEIGHT + BALL_RADIUS,
      ];
      if (SHOW_BOUND) {
        // DEBUG FLAG
    ...
    isColliedDownsideGap(bx, by, x_flip, y_flip, b_left) {
      ...
      let bound_x = [
        gap_centerX - BRICK_GAP_THRESHOLD / 2,
        gap_centerX + BRICK_GAP_THRESHOLD / 2,
      ];
      let bound_y = [
        refer_y + BRICK_HEIGHT,
    ...
    isColliedRightside(bx, by, x_flip, y_flip) {
      ...
      let bound_x = [
        refer_x + BRICK_WIDTH,
        refer_x + BRICK_WIDTH + BALL_RADIUS,
      ];
    ...
    isColliedRightsideGap(bx, by, x_flip, y_flip, b_down) {
      ...
      let bound_y = [
        gap_centerY - BRICK_GAP_THRESHOLD / 2,
        gap_centerY + BRICK_GAP_THRESHOLD / 2,
      ];
      if (SHOW_BOUND) {
        // DEBUG FLAG
    ...
    isColliedCorner(cornerDirection, bx, by, x_flip, y_flip) {
      ...
      if (areaCheck) {
        if (
          Math.pow(bx - cornerX, 2) + Math.pow(by - cornerY, 2) <
    ...
    updateColor(maxDurability) {
      ...
      const calcultateColorByPercent = (percent) => {
        ...
        const green_color = Math.floor(
          percent * green[1] + (1 - percent) * green[0]
        );
        const blue_color = Math.floor(
          percent * blue[1] + (1 - percent) * blue[0]
        );
        return (
          "#" +
          ((red_color & 0xff0000) | (green_color & 0x00ff00) | blue_color)
            .toString(16)
            .padStart(6, "0")
        );
      };

      const colorCode = calcultateColorByPercent(
        (this.durability - 1) / (maxDurability - 1)
      );
  ...
  class Item {
    constructor(index_x) {
      this.x =
        BRICK_MARGIN + index_x * (BRICK_WIDTH + BRICK_MARGIN) + BRICK_WIDTH / 2;
  ...
  class ScoreSystem {
    ...
    updateLB() {
      ...
      let leaderBoardMonth = this.leaderBoard.filter((elem) => {
        ...
        return (
          nowDate.getYear() === elemDate.getYear() &&
          nowDate.getMonth() === elemDate.getMonth()
        );
      });
